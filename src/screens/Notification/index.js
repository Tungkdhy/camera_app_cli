...
function Notification({ navigation }) {
    ...
    const formatTime = (date) => {
        ...
        const dateToTime = (hour <= 9 ? '0' + hour : hour) + ':' + (minute <= 9 ? '0' + minute : minute) + ':' + (seconds <= 9 ? '0' + seconds : seconds)
        return (dateToTime)
    }
    ...
    useEffect(() => {
        const getNotification = async () => {
            const res = await axiosClient.get(`notification/get-list-notification/?page=${page}&size=10`)
            const data = res.data;
            dispatch(setCountNotification(res.count_not_seen))
            const dataSmartReport = [];
            const dataSystemReport = [];
            data.forEach(notification => {
                if (notification.TYPE_NAME === 'AI') {
                    dataSmartReport.push(notification)
                } else {
                    dataSystemReport.push(notification)
                }
            });
    ...
    const seenNotification = async (codeItem) => {
        const res = await axiosClient.put(`notification/put-change-notification-seen/?notification_code=${codeItem}`)
    ...
    return (
        <View style={styles.container}>
            ...
            <View style={styles.control}>
                <Pressable
                    style={!smartReport ? { ...styles.button, ...styles.button_active } : { ...styles.button }}
                    onPress={() => setSmartReport(false)}
                >
                    <Text style={!smartReport ? { ...styles.sub_button, ...styles.sub_button_active } : { ...styles.button }}>Thông báo hệ thống</Text>
                </Pressable>
                <Pressable
                    onPress={() => setSmartReport(true)}
                    style={smartReport ? { ...styles.button, ...styles.button_active } : { ...styles.button }}
                >
                    <Text style={smartReport ? { ...styles.sub_button, ...styles.sub_button_active } : { ...styles.button }}>Cảnh báo thông minh</Text>
                </Pressable>
            </View>
            <View style={styles.content}>
                <ScrollView onScrollEndDrag={handleGetMoreNotification}>
                    {smartReport ? (
                        <>
                            {listNotificationSmart.length > 0 && listNotificationSmart.map((notification, index) => {
                                return (
                                    <View style={styles.date_block} key={index}>
                                        ...
                                        <View style={styles.list_item}>
                                            {notification.data.map((item) => {
                                                return (
                                                    <Pressable key={item.CODE} onPress={() => handleCheckNotification(item.CODE)}>
                                                        <View
                                                            style={item.SEEN === 0 ? { ...styles.item, ...styles.new } : { ...styles.item }}
                                                        >
                                                            <Text style={styles.time}>{item.TIME}</Text>
                                                            <Text style={styles.name}>{item.NAME}</Text>
                                                            <Text style={styles.title}>{item.DETAIL}</Text>
                                                            {item.SEEN === 0 && (<View style={styles.tick}></View>)}
                                                        </View>
                                                    </Pressable>
                                                )
                                            })}
                        ...
                        <>
                            {listNotificationSystem.length > 0 && listNotificationSystem.map((notification, index) => {
                                return (
                                    <View style={styles.date_block} key={index}>
                                        <Text style={styles.time_block}>{notification.TIME}</Text>
                                        <View style={styles.list_item}>
